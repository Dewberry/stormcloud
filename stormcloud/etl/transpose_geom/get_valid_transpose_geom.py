...
def create_dummy_transposer(
    watershed_uri: str, transposition_uri: str, session, **kwargs
) -> Transposer:
    ...
    if kwargs.get("pickled_dataset_filepath"):
        ...
        xsum = get_xr_dataset(
            DATA_TYPE, START_DATE, DURATION, AGG_METHOD, transposition_geom
        )
...
def main(
    watershed_uri: str, transposition_uri: str, out_geojson_path: str, session, **kwargs
):
    transposer = create_dummy_transposer(
        watershed_uri, transposition_uri, session, **kwargs
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "watershed_uri",
        type=str,
        help="s3 URI of geojson associated with watershed of interest",
    )
    parser.add_argument(
        "transposition_uri",
        type=str,
        help="s3 URI of geojson associated with transposition domain of interest",
    )
    parser.add_argument(
        "out_geojson_path",
        type=str,
        help="output path for geojson of valid transposition areas",
    )
    parser.add_argument(
        "--p",
